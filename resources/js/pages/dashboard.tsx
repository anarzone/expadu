...
type WeatherProps = { temperature: number; emoji: string; condition: string; wind_speed: number; feels_like: number };
...
type DashboardFeed = {
    ...
    settlement: { total: number; completed: number; percent: number; days_since_arrival: number; situation: string | null };
    departures: { stop_name: string; source: string; departures: Array<{ line: string; direction: string; color: string; type: string; departures: number[] }> };
    places: Array<{ id: number; emoji: string | null; name: string; address: string | null }>;
    nearby_spots: NearbySpot[];
    this_week: Array<{ id: number; title: string; emoji: string | null; category: string; starts_at: string; location_name: string | null; is_free: boolean }>;
};
...
export default function Dashboard() {
    type CommuteRec = {
        ...
        to: string;
        headline: string;
        route_cards: Array<{ type: string; badge: string; name: string; detail: string; time: number; status: string; best: boolean; to_lat?: number; to_lng?: number; to_name?: string; link?: string }>;
    ...
    const [routeSheetDest, setRouteSheetDest] = useState<{ name: string; emoji: string; lat: number; lng: number } | null>(null);

    // Split recommendations by purpose
    const topAlert = recs.find((r) => r.type === 'appointment' || r.type === 'deadline_overdue');
    const warnings = recs.filter((r) => ['deadline_overdue', 'deadline_warning', 'disruption', 'weather_alert'].includes(r.type) && r !== topAlert);
    const timelineRows = recs.filter((r) => !['deadline_overdue', 'deadline_warning', 'disruption', 'weather_alert', 'appointment'].includes(r.type)).slice(0, 3);
            ...
            <div className="mx-auto w-full max-w-[680px]">
                ...
                {weather && (
                    <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2DFD6' }}>
                        ...
                        >
                            <div className="pointer-events-none absolute" style={{ top: -50, right: -50, width: 180, height: 180, background: 'rgba(255,255,255,.05)', borderRadius: '50%' }} />

                            {/* Eyebrow — commute context */}
                            <div className="relative z-[1]" style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.10em', opacity: 0.6, marginBottom: 6 }}>
                                {cr?.context === 'off_hours' || cr?.context === 'discovery'
                            ...
                            {topAlert && (
                                <div className="relative z-[1]" style={{ marginBottom: 14 }}>
                                    <div
                                        className="flex cursor-pointer items-center gap-3 rounded-[9px] transition-all hover:bg-[rgba(255,255,255,.22)]"
                                        style={{ background: 'rgba(255,255,255,.15)', border: '1px solid rgba(255,255,255,.2)', padding: '12px 14px' }}
                                    >
                                        <span style={{ fontSize: 22, flexShrink: 0 }}>{topAlert.emoji}</span>
                                        <div className="min-w-0 flex-1">
                                            {topAlert.type === 'appointment' && topAlert.meta?.label && (
                                                <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', opacity: 0.65, marginBottom: 3 }}>
                                                    ⚠️ {topAlert.meta.urgent ? 'Urgent' : 'Upcoming'} · {String(topAlert.meta.label)}
                                                </div>
                                            )}
                                            {topAlert.type === 'deadline_overdue' && (
                                                <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', opacity: 0.65, marginBottom: 3 }}>
                                                    🚨 Overdue
                                                </div>
                                            )}
                                            <div style={{ fontSize: 14, fontWeight: 700, lineHeight: 1.2 }}>{topAlert.title}</div>
                                            <div style={{ fontSize: 11, opacity: 0.75, marginTop: 2 }}>{topAlert.subtitle}</div>
                                        </div>
                                        {topAlert.value && (
                                            <div className="shrink-0 text-right">
                                                <div style={{ fontFamily: "'Geist Mono', monospace", fontSize: 16, fontWeight: 600, lineHeight: 1 }}>{topAlert.value}</div>
                                                <div style={{ fontSize: 9, opacity: 0.6, display: 'block' }}>{topAlert.unit}</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="relative z-[1]" style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 400, lineHeight: 1.2, marginBottom: 16 }}>
                                {cr?.headline ?? `${weather.temperature}°C, ${weather.condition.toLowerCase()}.`}
                            </div>
                            ...
                            {needsSetup && (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <div style={{ fontSize: 13, fontWeight: 600 }}>Set your Home &amp; Work addresses</div>
                                        <div style={{ fontSize: 11, opacity: 0.7 }}>For personalized commute &amp; route recommendations</div>
                                    </div>
                                    <span style={{ fontSize: 12, fontWeight: 600, opacity: 0.7 }}>→</span>
                                </a>
                            )}

                            {/* Divider */}
                            <div className="relative z-[1]" style={{ height: 1, background: 'rgba(255,255,255,.12)', marginBottom: 12 }} />
                            ...
                            <div className="relative z-[1] flex flex-col" style={{ gap: 8 }}>
                                {(cr?.route_cards ?? []).map((card, i) => {
                                    ...
                                    return <RotatingCardSlot key={i} pool={pool} onCardClick={(c) => {
                                        if (c.to_lat && c.to_lng) {
                                            setRouteSheetDest({ name: String(c.to_name ?? c.name ?? ''), emoji: String(c.badge ?? '📍'), lat: Number(c.to_lat), lng: Number(c.to_lng) });
                                        }
                                    }} />;
                                })}
                            </div>
                            ...
                            {/* Leave-by */}
                            {cr?.leave_by && (
                                <div className="relative z-[1] mt-[10px] flex items-center gap-[10px]" style={{ background: 'rgba(255,255,255,.15)', borderRadius: 9, padding: '11px 14px' }}>
                                    <span style={{ fontSize: 18, flexShrink: 0 }}>⏰</span>
                                    <div style={{ fontSize: 13, lineHeight: 1.4, flex: 1 }}>
                                        Leave by <strong style={{ fontWeight: 700 }}>{cr.leave_by.time}</strong> to arrive on time. {cr.leave_by.message}
                                    </div>
                                </div>
                            )}
                        </div>
                                                ...
                                                {spot.tags.map((tag) => {
                                                    ...
                                                    return (
                                                        <span
                                                            ...
                                                            style={{
                                                                fontSize: 10, padding: '2px 6px', borderRadius: 20, fontWeight: 500,
                                                                background: t?.bg ?? '#EFEDE7',
                                                                color: t?.color ?? '#6B6860',
                                                            }}
                                                        >
                                                            {TagIcon && <TagIcon size={10} stroke={ICON_STROKE} />}
                                                            {t?.label ?? tag}
                                                        </span>
                                                    );
                                                })}
                        ...
                        <div className="flex flex-col gap-2">
                            {thisWeek.map((ev) => (
                                ...
                                >
                                    ...
                                    {ev.is_free && (
                                        <span className="shrink-0 rounded-full bg-[#D4F0E6] px-2 py-0.5 text-[9px] font-bold uppercase text-[#0A7C52]">Free</span>
                                    )}
                                </a>
                            ))}
                        </div>
                ...
                <div style={{ padding: '20px 24px' }}>
                    <div className="grid grid-cols-4 gap-2">
                        {[
                            ...
                            { icon: IconFileText, label: 'Checklist', href: '/bureaucracy' },
                            { icon: IconLanguage, label: 'Language', href: '/language-exchange' },
                            { icon: IconBell, label: 'Alerts', href: '/alerts' },
                        ].map((item) => (
                            <a
                                key={item.label}
                                href={item.href}
...
function RecCard({ rec }: { rec: Recommendation }) {
    const colorMap: Record<string, { bg: string; text: string; border: string }> = {
        danger: { bg: '#FDE8E6', text: '#C4271A', border: 'rgba(196,39,26,.15)' },
        warn: { bg: '#FDF0D4', text: '#C47D0E', border: 'rgba(196,125,14,.15)' },
        success: { bg: '#D4F0E6', text: '#0A7C52', border: 'rgba(10,124,82,.15)' },
        accent: { bg: '#EBF0FD', text: '#1A4CD4', border: 'rgba(26,76,212,.15)' },
...
function RotatingCardSlot({ pool, onCardClick }: { pool: Array<Record<string, unknown>>; onCardClick: (card: Record<string, unknown>) => void }) {
    ...
    useEffect(() => {
        ...
        function scheduleNext() {
            ...
            timer = setTimeout(() => {
                setFlipping(true);
                setTimeout(() => { setIndex((prev) => (prev + 1) % pool.length); setFlipping(false); scheduleNext(); }, 300);
            }, delay);
        }
        scheduleNext();
    ...
    return (
        <div style={{ transition: 'transform 0.3s ease, opacity 0.3s ease', transform: flipping ? 'rotateX(90deg)' : 'rotateX(0deg)', opacity: flipping ? 0 : 1, transformOrigin: 'center center' }}>
            <RouteOptionCard
...
function RouteOptionCard({ badge, badgeMono, name, detail, time, statusColor, best, onClick }: {
    badge: string; badgeMono?: boolean; name: string; detail: string; time: number; statusColor: string; best?: boolean; onClick?: () => void;
}) {
    return (
        <div onClick={onClick} className="flex cursor-pointer items-center transition-all hover:bg-[rgba(255,255,255,.18)]" style={{ background: best ? 'rgba(255,255,255,.20)' : 'rgba(255,255,255,.12)', borderRadius: 9, padding: '12px 14px', gap: 12, border: best ? '2px solid rgba(255,255,255,.4)' : '2px solid transparent' }}>
            <div className="flex shrink-0 items-center justify-center" style={{ width: 34, height: 34, borderRadius: 8, background: 'rgba(255,255,255,.2)', fontSize: badgeMono ? 13 : 16, fontFamily: badgeMono ? "'Geist Mono', monospace" : undefined, fontWeight: badgeMono ? 700 : undefined }}>{badge}</div>
            <div className="min-w-0 flex-1">
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 1 }}>
                    {name}
                    {best && <span style={{ fontSize: 9, fontWeight: 700, background: 'rgba(255,255,255,.25)', padding: '2px 7px', borderRadius: 20, marginLeft: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>⭐ Best</span>}
                </div>
