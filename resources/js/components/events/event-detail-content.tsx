...
export function EventDetailContent({ event, going, onRsvp }: { event: EventData; going: boolean; onRsvp: () => void }) {
    const attendStr = formatAttending(event.attending);
    const spotsLeft = event.max && event.max < 9999 ? event.max - event.attending : null;
    ...
    return (
        <>
            ...
            {/* Title */}
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 500, lineHeight: 1.2, marginBottom: 8 }}>
                {event.title}
            </div>

            {/* Organiser */}
            <div style={{ fontSize: 13, color: '#6B6860', marginBottom: 16 }}>Organised by {event.organiser}</div>

            {/* Detail rows */}
            <div className="mb-4 flex flex-col gap-2">
                <DetailRow icon="📅" text={`${event.day} ${event.date} ${event.month} · ${event.time} · ${event.duration}`} />
                <DetailRow icon="📍" text={`${event.venue}, ${event.area} · ${event.distance}`} />
                <DetailRow icon="💶" text={event.price} />
                <DetailRow icon="👥" text={`${attendStr} attending${spotsLeft !== null ? ` · ${spotsLeft} spots left` : ''}`} />
            </div>
            ...
            <div style={{ fontSize: 14, color: '#6B6860', lineHeight: 1.65, marginBottom: 16 }}>{event.desc}</div>
            ...
            <div className="mb-5 flex flex-wrap gap-[5px]">
                {event.tags.map((t) => (
                    <span
                        ...
                        style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.03em', padding: '3px 10px', background: t.bg, color: t.c }}
                    >
                        {t.l}
                    </span>
                ))}
                {event.englishFriendly && (
                    <span
                        ...
                        style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', background: '#D4F0E6', color: '#0A7C52' }}
                    >
                        🇬🇧 English-friendly
                    </span>
                )}
            ...
            <div className="flex gap-[9px]">
                ...
                {event.link && event.link !== '#' && (
                    <button
                        onClick={() => window.open(event.link, '_blank', 'noopener')}
                        className="cursor-pointer rounded-[9px] border border-[#E2DFD6] bg-[#EFEDE7] px-4 py-[13px] transition-all hover:bg-[#E2DFD6]"
                        style={{ fontFamily: "'Geist', sans-serif", fontSize: 13, fontWeight: 600 }}
                    >
                        Website ↗
