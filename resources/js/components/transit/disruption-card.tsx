export function DisruptionCard({
    ...
    return (
        <div className={`flex items-start gap-2.5 rounded-lg border p-3 ${bg}`}>
            ...
            <div
                className={`min-w-0 flex-1 text-[13px] leading-relaxed ${textColor}`}
            >
                ...
                <div className="mt-1 flex gap-1.5">
                    {disruption.lines.map((l) => (
                        <span
                            key={l}
                            className="rounded bg-white/60 px-1.5 py-0.5 text-[10px] font-bold dark:bg-black/20"
                        >
                            {l}
                        </span>
                    ))}
            ...
            {onDismiss && (
                <button
                    onClick={() => onDismiss(disruption.id)}
