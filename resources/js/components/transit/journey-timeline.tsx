    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (seg.type === 'walk') {
            ...
            if (i === segments.length - 1) {
                const walkEnd =
                    seg.duration_min && currentTime
                        ? addMinutes(currentTime, seg.duration_min)
                        : '';
                nodes.push({
                    type: 'stop',
                    time: walkEnd,
        ...
        <div className="flex items-center gap-3" style={{ minHeight: 40 }}>
            ...
            <div className="flex w-5 flex-col items-center">
                <div
                    ...
                    style={{
                        border: `3px solid ${isFirst ? '#0A7C52' : isLast ? '#C4271A' : 'currentColor'}`,
                    }}
                />
            </div>
        ...
        <div className="flex gap-3" style={{ minHeight: 48 }}>
            ...
            <div className="flex w-5 flex-col items-center">
                <div
                    ...
                    style={{
                        width: 3,
                        background:
                            'repeating-linear-gradient(to bottom, #1A4CD4 0px, #1A4CD4 4px, transparent 4px, transparent 10px)',
                    }}
                />
            </div>
            <div className="flex items-center gap-2 py-2">
                <IconWalk
                    size={16}
                    stroke={ICON_STROKE}
                    className="shrink-0 text-[#6B6860] dark:text-[#AAA89F]"
                />
                <div>
                    <div className="text-[13px] text-[#6B6860] dark:text-[#AAA89F]">
                        Walk
                    </div>
                    <div className="text-xs text-[#AAA89F] dark:text-[#6B6860]">
                        About {duration} min
                    </div>
                </div>
            </div>
...
function TransitLeg({
    ...
    const TransitIcon =
        mode === 'rail' ? IconTrain : mode === 'bus' ? IconBus : IconTrain;
            ...
            <div className="flex items-center gap-2 py-2">
                ...
                <div>
                    <div className="flex items-center gap-1.5">
                        <span
                            className="inline-flex items-center rounded px-[6px] py-[1px] font-mono text-xs font-bold text-white"
                            style={{ background: color }}
                        >
                            {line}
                        </span>
                        <span className="text-[13px] text-[#18170F] dark:text-[#F5F4F0]">
                            {direction}
                        </span>
                    </div>
                    <div className="mt-[2px] text-xs text-[#AAA89F] dark:text-[#6B6860]">
                        ...
                        {(delay ?? 0) > 0 && (
                            <span className="font-semibold text-[#C47D0E]">
                                {' '}
                                · +{delay}min late
                            </span>
                        )}
