    ...
    return (
        <div className="relative flex-1">
            <div
                ...
                style={{ background: '#FFFFFF', border: '1px solid #E2DFD6', borderRadius: 9, padding: '11px 14px' }}
            >
                <span style={{ fontSize: 15, color: '#AAA89F', flexShrink: 0 }}>{icon}</span>
                <input
                ...
                {loading && (
                    <div
                        style={{
                            width: 14, height: 14,
                            border: '2px solid #E2DFD6', borderTopColor: '#1A4CD4',
            ...
            {dropdownOpen && results.length > 0 && (
                <div
                    style={{
                        ...
                        background: 'white', border: '1px solid #E2DFD6', borderRadius: 9,
                        boxShadow: '0 8px 24px rgba(0,0,0,0.08)', zIndex: 60, overflow: 'hidden',
                    }}
                >
                    {results.map((r, idx) => (
                        <div
                            ...
                            onMouseDown={(e) => {
                                e.preventDefault();
                                const label = [r.name, r.street, r.city].filter(Boolean).join(', ');
                                onChange(label);
                                onSelect({ lat: r.lat, lng: r.lng, name: r.name, label });
                                setDropdownOpen(false);
                                setResults([]);
                            }}
                            ...
                            style={{
                                padding: '10px 14px',
                                borderBottom: idx < results.length - 1 ? '1px solid #F0EDE7' : 'none',
                            }}
                        >
                            <div style={{ fontSize: 14, fontWeight: 500, color: '#18170F' }}>{r.name}</div>
