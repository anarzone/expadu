 ...
 * quietly borrowing the bike figure. A one-to-many street matrix cannot provide
 * a real transit duration, so transit returns no card time until the user opens
 * a full journey in take-me-there. An unset preference defaults to walk, which
 * is what the From control shows.
 */
class TravelTimes
{
    ...
    public function minutes(?TransportMode $mode, GeoPoint $origin, array $destinations): array
    {
        ...
        // Walk — and the unset default — show the real walking time, so a card
        // never shows another mode's figure under a walk or transit label.
        $profile = $mode === TransportMode::Bike ? 'BIKE' : 'WALK';
